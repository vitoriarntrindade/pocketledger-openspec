...
def test_same_name_allowed_across_different_types(client, auth_headers):
    client.post(
        "/api/v1/categories", json={"name": "Outros", "type": "expense"}, headers=auth_headers
    )
    response = client.post(
        "/api/v1/categories", json={"name": "Outros", "type": "income"}, headers=auth_headers
    )
    assert response.status_code == 201
...
def test_rename_success(client, auth_headers):
    ...
    response = client.patch(
        f"/api/v1/categories/{created['id']}", json={"name": "Comida"}, headers=auth_headers
    )
    assert response.status_code == 200
...
def test_rename_to_duplicate_rejected(client, auth_headers):
    client.post(
        "/api/v1/categories", json={"name": "Comida", "type": "expense"}, headers=auth_headers
    )
    created = client.post(
        "/api/v1/categories", json={"name": "Alimentacao", "type": "expense"}, headers=auth_headers
    ).json()
...
def test_delete_unused_category_succeeds(client, auth_headers):
    created = client.post(
        "/api/v1/categories", json={"name": "Lazer", "type": "expense"}, headers=auth_headers
    ).json()
    response = client.delete(f"/api/v1/categories/{created['id']}", headers=auth_headers)
...
def test_cross_user_category_access_rejected(client, auth_headers, other_auth_headers):
    category = client.post(
        "/api/v1/categories", json={"name": "Alimentacao", "type": "expense"}, headers=auth_headers
    ).json()
...
def test_listing_scoped_to_owner(client, auth_headers, other_auth_headers):
    ...
    client.post(
        "/api/v1/categories", json={"name": "Transporte", "type": "expense"}, headers=other_auth_headers
    )
